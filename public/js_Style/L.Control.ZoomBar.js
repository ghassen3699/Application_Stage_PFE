...
L.Control.ZoomBar = L.Control.extend({
    ...
    onAdd: function(map) {
        ...
        this._zoomStartButton = this._createButton('<i class="fa fa-location-arrow"></i>', 'Zoom to Start Position',
            'leaflet-control-zoom-to-start', container, this._zoomStart, this);

        this._zoomInButton = this._createButton('<i class="fa fa-plus-square"></i>', 'Zoom In',

            //this._zoomInButton  = this._createButton('<i class="fas fa-search-plus"></i>', 'Zoom In',
            'leaflet-control-zoom-to-start', container, this._zoomIn, this);

        this._zoomOutButton = this._createButton('<i class="fa fa-minus-square"></i>', 'Zoom Out',
            'leaflet-control-zoom-to-start', container, this._zoomOut, this);

        // The ZoomToArea button doesn't work well on mobile so don't add it.
        if (this._isSmallScreen() === false) {
            this._zoomToAreaButton = this._createButton('<i class="fa fa-search"></i> ', 'Zoom to Area',
                'leaflet-control-zoom-to-start', container, this._zoomToArea, this);
        }


        /*
		this._zoomStartButton  = this._createButton('<i class="fas fa-search-location"></i>', 'Zoom to Start Position',
                'leaflet-control-zoom-to-start',  container, this._zoomStart,  this);
                
        this._zoomInButton  = this._createButton('<i class="fas fa-plus-square"></i>', 'Zoom In',
                'leaflet-control-zoom-in',  container, this._zoomIn,  this);
                
        this._zoomOutButton = this._createButton('<i class="fas fa-minus-square"></i>', 'Zoom Out',
                'leaflet-control-zoom-out', container, this._zoomOut, this);
                
        // The ZoomToArea button doesn't work well on mobile so don't add it.
        if( this._isSmallScreen()===false ){
            this._zoomToAreaButton = this._createButton('<i class="fas fa-clone"></i>', 'Zoom to Area', 
